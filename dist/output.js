...
export function scoreColor(score) {
    if (score === null || score < 0)
        return chalk.gray("--");
    if (score >= 80)
        return chalk.green(String(score).padStart(2));
    if (score >= 50)
        return chalk.yellow(String(score).padStart(2));
...
export function verdictText(result) {
    switch (result?.toLowerCase()) {
        ...
        case "fail":
        case "unsafe": return chalk.red("UNSAFE");
        default: return chalk.gray("UNKNOWN");
    }
}
export function formatPackageLine(name, pkg, type) {
    if (!pkg) {
...
export function formatPackageDetail(name, pkg) {
    const lines = [];
    const icon = verdictIcon(pkg.latest_result);
    ...
    if (pkg.source_url)
        lines.push(`   Source:       ${chalk.dim(pkg.source_url)}`);
...
export function formatSummary(total, passed, warned, failed, unknown) {
    const parts = [];
