...
Examples:
  ...
  .action(async (opts) => {
    ...
    if (packages.length === 0) {
      ...
      if (sources.npm) console.log(chalk.dim("    • package.json"));
      if (sources.pip) console.log(chalk.dim("    • requirements.txt / pyproject.toml"));
    ...
    // Group by source
    const bySource = new Map<string, typeof packages>();
    ...
    const results: { name: string; source: string; found: boolean; pkg: any }[] = [];
    ...
    const order: Record<string, number> = { unsafe: 0, fail: 0, warn: 1, pass: 2, safe: 2 };
    results.sort((a, b) => {
    ...
    if (opts.json) {
      ...
      for (const r of results) {
        if (!r.found) { unknown++; continue; }
        const v = r.pkg?.latest_result?.toLowerCase();
        if (v === "pass" || v === "safe") passed++;
