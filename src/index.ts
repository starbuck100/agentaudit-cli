...
program
  ...
  .action(async (opts) => {
    ...
    try {
      const result: ScanResult = await scan(opts.config || []);
      ...
      for (const pkg of result.packages) {
        ...
        if (pkg.trustScore >= 0 && pkg.trustScore < 40) hasFailure = true;
      }
      ...
      if (warned > 0) console.log(chalk.yellow(`  ⚠️  ${warned} warnings`));
      if (failed > 0) console.log(chalk.red(`  🔴 ${failed} failed`));
      if (errors > 0) console.log(chalk.dim(`  ❓ ${errors} could not be checked`));
      console.log(chalk.dim("───────────────────────────────────────\n"));
...
function printPackageResult(pkg: PackageResult) {
  if (pkg.error) {
  ...
  let icon: string, label: string, colorFn: (s: string) => string;

  if (score >= 70) {
    icon = "✅"; label = "PASS"; colorFn = chalk.green;
  } else if (score >= 40) {
    icon = "⚠️"; label = "WARN"; colorFn = chalk.yellow;
  } else {
    icon = "🔴"; label = "FAIL"; colorFn = chalk.red;
  }
